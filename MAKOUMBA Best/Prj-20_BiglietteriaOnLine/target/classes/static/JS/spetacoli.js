...
function leggiSpetacoli(){ 
    ...
    .then(response => {
        ...
        response.forEach(spetacolo => {
            rowCont1.appendChild(creaCard(spetacolo));
        });
    })
}
...
cardBody.setAttribute("class", "card-body");
  ...
  cardBody.innerHTML  = `<h3 class="card-title">${spetacolo.titolo}</h3>
      ...
      let btnCompra = document.createElement("button");
                ...
                btnCompra.textContent = "Prenota";

                //richiama acquista per usare click btn buy
...
function acquista(spetacolo){

    console.log(spetacolo)
